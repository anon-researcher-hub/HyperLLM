"""
Ablation Study 2: Remove Network Optimizer (OptimizerAgent)
Disable optimizer functionality, observe network evolution without global optimization guidance
"""
import sys
import os
...
class NoOptimizerMASGenerator(ProtectedMASHypergraphGenerator):
    """MAS hypergraph generator with optimizer disabled"""
    
    def run_iteration(self, iteration: int) -> Dict[str, Any]:
        """Override iteration method, completely skip optimizer logic"""
        try:
            print(f"\n🔄 Ablation-NoOptimizer Iter {iteration + 1}/{self.num_iterations}")
            ...
            # 1. Optimizer agent - completely disabled
            print("🚫 Ablation: Optimizer disabled")
            iteration_results['actions'].append({
                'action': 'optimize',
                'agent_id': 'optimizer_disabled',
                'strategy': 'DISABLED',
                'reasoning': 'Ablation: optimizer functionality disabled'
            })
            
            # 2. Relationship remover agent (keep)
                ...
                if len(generator_decision['selected_members']) > 1:
                    if is_building_phase:
                        ...
                        if should_approve:
                            ...
                            print(f"✅ Add edge(NoOptimizer, size {len(generator_decision['selected_members'])}): {' '.join(generator_decision['selected_members'])}")
                            generated_count += 1
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Ablation Study 2: MAS Hypergraph Generator without Optimizer")
    ...
    generator = NoOptimizerMASGenerator(
